...
const {
    ...
    uniqueDnaTorrance,
    editionSize,
} = require(`${basePath}/src/logic/config.js`);
...
const startCreating = async () => {
    ...
    while (editionCount <= editionSize)
    {
        ...
        if (isDnaUnique(dnaList, newDna))
        {
            ...
            await Promise.all(loadedElements).then((elementArray) => {
                ...
                var base64 = saveImage(editionCount);
                addMetadata(newDna, editionCount);
                var jsonContent = saveMetaDataSingleFile(editionCount);
                console.log(`Created edition: ${editionCount}, with DNA: ${newDna}`);
                console.log("testing 1");
                datoPadre = base64;
                textPadre = jsonContent;
            });
            //console.log(datoPadre)
            sanPadre = datoPadre;

            //console.log(sanPadre)
            //console.log(datoPadre)
            console.log("testing");
            dnaList.push(newDna);
            editionCount++;
    ...
    console.log(textPadre);

    return { dat, textPadre };
};
